...
export const MIGRATIONS: Migration[] = [
  ...
  },
  {
    ...
    // v0.18.1 RLS hardening: 12 gbrain-managed public tables shipped
    ...
    // subagent_rate_leases, gbrain_cycle_locks, sources,
    // file_migration_ledger, budget_ledger, budget_reservations). Supabase exposes the public schema via
    ...
    // wave. Older deployed brains may have had `sources` and `file_migration_ledger`
    // created before the schema.sql RLS block covered them, so v24 backfills
    // them too instead of assuming the base schema already protected them.
    //
    ...
    sql: `
      ...
      BEGIN
        ...
        END IF;
        IF EXISTS (SELECT 1 FROM information_schema.tables
                    WHERE table_schema = 'public' AND table_name = 'file_migration_ledger') THEN
