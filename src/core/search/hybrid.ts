...
export async function hybridSearch(
  ...
  if (!process.env.OPENAI_API_KEY) {
    let keywordOnly = dedupResults(keywordResults);
    keywordOnly = applySlugWeights(keywordOnly);
    ...
    if (keywordOnly.length > 0) {
      try {
        const slugs = Array.from(new Set(keywordOnly.map(r => r.slug)));
        const counts = await engine.getBacklinkCounts(slugs);
        applyBacklinkBoost(keywordOnly, counts);
        keywordOnly.sort((a, b) => b.score - a.score);
      } catch {
        // Boost failure is non-fatal: keep weighted ranking.
      }
    }

    return applyGraphExpansion(engine, keywordOnly, {
      graphDepth,
      offset,
      limit,
      innerLimit,
