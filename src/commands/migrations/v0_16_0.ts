...
async function phaseASchema(opts: OrchestratorOpts): Promise<OrchestratorPhaseResult> {
  ...
  try {
    ...
    if (!config) {
      return { name: 'schema', status: 'skipped', detail: 'no brain configured (run gbrain init first)' };
    }
    const engine = await createEngine(toEngineConfig(config));
    await engine.connect(toEngineConfig(config));
    try {
      const hasMinionJobs = await engine.executeRaw<{ exists: boolean }>(
        `SELECT EXISTS (
           SELECT 1 FROM information_schema.tables
           WHERE table_schema = current_schema() AND table_name = 'minion_jobs'
         ) AS exists`,
      );
      if (!hasMinionJobs[0]?.exists) {
        return { name: 'schema', status: 'skipped', detail: 'minion_jobs not yet created (fresh install)' };
      }
      await engine.executeRaw(SUBAGENT_SCHEMA_SQL);
