...
export async function runDoctor(engine: BrainEngine | null, args: string[], dbSource?: DbUrlSource) {
  ...
  if (repoInfo?.layout === 'skills') {
    // --fix: run auto-repair BEFORE checkResolvable so the post-fix scan reflects the new state.
    // Auto-fix only targets DRY violations today; other resolver issues are left to human repair.
    if (doFix) {
      autoFixReport = autoFixDryViolations(repoInfo.skillsDir, { dryRun });
